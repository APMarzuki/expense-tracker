...
users = ["Agus", "Zsolt", "Zoli", "Beata", "Thomas", "Marcel", "Kristof"]
...
def add_expense():
    ...
    try:
        ...
        if not category or not description or not user:
            messagebox.showerror("Input Error", "Category, Description, and User cannot be empty.")
            return
...
def modify_expense():
    ...
    user_combobox.set(expense_to_modify["user"])
...
def save_modified_expense():
    ...
    try:
        ...
        if not new_category or not new_description or not new_user:
            messagebox.showerror("Input Error", "Category, Description, and User cannot be empty.")
            return
...
def update_expense_list():
    ...
    if expenses:
        for exp in expenses:
            display_text = f"{exp['amount']:.2f} HUF - {exp['category']} - {exp['description']} - by {exp['user']}"
...
def filter_expenses(event):
    ...
    filtered_expenses = [
        ...
        if query in exp["category"].lower() or query in exp["description"].lower() or query in exp["user"].lower()
    ]
...
user_combobox.grid(row=3, column=1)

add_button = ttk.Button(input_frame, text="Add Expense", command=add_expense)
add_button.grid(row=4, column=0, columnspan=2, pady=10)

ttk.Label(input_frame, text="Search:").grid(row=5, column=0, sticky=tk.W, pady=(10, 0))
search_entry = ttk.Entry(input_frame, width=20)
search_entry.grid(row=5, column=1, pady=(10, 0))
